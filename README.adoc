= Across Documentation - Antora UI
// Settings:
:experimental:
:hide-uri-scheme:
// Project URLs:
...
=== Merging the upstream Antora UI into this project

Our Github repository contains customizations for the Foreach CSS templates.
To merge the upstream Antora UI commits into this project, use the following commands:

 $ git remote add upstream https://gitlab.com/antora/antora-ui-default.git
 $ git fetch upstream
